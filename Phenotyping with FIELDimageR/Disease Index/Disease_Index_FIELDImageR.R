...
#set directory, where the folder is located
# Images names (folder directory: "./EX2/")
pics<-list.files("./EX2/")
pics
...
#Choose one [first] image to prepare the pipeline
EX.L1<-stack(paste("./EX2/",pics[1],sep = ""))  #skip warnings 
plotRGB(EX.L1)


# Shapefile with extent = T (The whole image area will be the shapefile)
# you need to indicate 4 corner points if extent = F
EX.L.Shape<-fieldPolygon(mosaic=EX.L1, extent = T) 


# Select one index to identify leaves and remove the background
#cropAbove=T means the leaf/crop is above the page/background
